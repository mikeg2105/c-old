	...
	#include "../include/ising_system.hpp"

	CIsing_System::CIsing_System(int iside)
	{
		m_iside=iside;
		m_Nparticles=m_iside*m_iside;
		m_particles=new CIsing_Particle [m_Nparticles];
		m_h=0.1;							
	}
		
	CIsing_System::~CIsing_System()
	{
		delete [] m_particles;
		cout << "Monte-carlo simulation completed. Good bye." << endl<<endl;			
	}
	
	void CIsing_System::perturb()
	{
		m_current=m_Nparticles*randno();
		m_particles[m_current].flip_spin();
	}
	
	void CIsing_System::restore()
	{
		m_particles[m_current].flip_spin();	
	}
	
	double CIsing_System::energy()
	{
		find_neighbours();
	...
	double CIsing_System::magnetisation()
	{
		double mag=0;
	...
	void CIsing_System::find_neighbours()
	{
